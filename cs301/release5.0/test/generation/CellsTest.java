...
public class CellsTest {
	...
	/**
	 * We create a default (width x height) cells object that is not initialized and
	 * a (1x1) cells1 object.
	 */
	@Before
	public void setUp() {
	...
	/**
	 * Test case: Check if constructor that takes existing array really copies
	 * values and resets values with the initialize method.
	 * <p>
	 * Method under test: Cells(int[][] input), getValueOfCell(int i, int j)
	 * <p>
	 * Correct behavior: constructor delivers a cells object where all internal
	 * positions are set as given. After initialization those values must be set
	 * differently.
	 */
	@Test
	 ...
	 * <p>
	 * Method under test: Cells(int[][] input), Cells(width,height), equals(Object
	 * other)
	 * <p>
	 * Correct behavior: It is correct if each constructor delivers a cells object
	 * and that both are equal if of same dimension and of same content
	 */
	@Test
	public final void testCellsBothConstructors() {
		...
		// inner walls being up, such that values can not be 0 anywhere after
		// initialization
	 ...
	 * <p>
	 * Correct behavior: checks if adjacent cells (x,y) and its neighbor (x+dx,y+dy)
	 * are not separated by a border and (x+dx,y+dy) has not been visited before.
	 */
	@Test
	public final void testCanGo() {
		assertTrue(width > 1);
		assertTrue(height > 1);
		// the initial 4x4 cells have walls up everywhere, but borders only on the
		// outside
	 ...
	 * <p>
	 * Method under test: setExitPosition(int x, int y) and isExitPosition(int x,
	 * int y, int bit)
	 * <p>
	 * Correct behavior: it sets a given bit to zero in a given cell
	 */
	@Test
	 ...
	 * <p>
	 * Correct behavior: Method sets visited flag to zero for a given cell.
	 */
	@Test
	 ...
	 * <p>
	 * Correct behavior: it sets the InRoom bit to one for a given cell and
	 * direction
	 */
	@Test
	 ...
	 * <p>
	 * Correct behavior: Initialize maze such that all cells have not been visited
	 * (CW_VIRGIN), all walls are up (CW_ALL), and borders are set as a rectangle
	 * (CW_*_BOUND).
	 */
	@Test
	 ...
	 * <p>
	 * Correct behavior: Checks if there is a cell in the given area that belongs to
	 * a room. The first corner is at the upper left position, the second corner is
	 ...
	 * <p>
	 * Method under test: deleteWall(int x, int y, CardinalDirection cd) Method
	 * under test: addWall(int x, int y, CardinalDirection cd)
	 * <p>
	 * Correct behavior: it deletes a wall between to adjacent cells (x,y) and
	 * (x+dx,y+dy).
	 */
	@Test
	 ...
	 * <p>
	 * Method under test: markAreaAsRoom(int rw, int rh, int rx, int ry, int rxl,
	 * int ryl, Random r) Method under test: isInRoom(int x, int y)
	 * <p>
	 * Correct behavior: it marks a given area as a room on the maze and positions
	 * up to five doors randomly. The first corner is at the upper left position,
	 * the second corner is at the lower right position. Assumes that given area is
	 * located on the map and does not intersect with any existing room. The walls
	 * of a room are declared as borders to prevent the generation mechanism from
	 * tearing them down. rw is the room width, rh is the room height, rx is 1st
	 * corner, x coordinate, ry is 1st corner, y coordinate, rxl is 2nd corner, x
	 * coordinate, ryl is 2nd corner, y coordinate
	 */
	@Test
	 ...
	 * <p>
	 * Correct behavior: it gets methods (is..., has...) for various flags
	 */
	@Test
	 ...
	 * <p>
	 * Correct behavior: it tells if InRoom flag is set for given cell
	 */
	@Test
	 ...
	 * <p>
	 * Method under test: hasWall(int x, int y, CardinalDirection d) Method under
	 * test: hasNoWall(int x, int y, CardinalDirection)
	 * <p>
	 * Correct behavior: it tells if cell has a wall in the given direction
	 */
	@Test
	 ...
	 * <p>
	 * Correct behavior: it tells if masked bit is false
	 */
	@Test
	 ...
	 * <p>
	 * Correct behavior: it dumps internal data into a string, intended usage is for
	 * debugging purposes. Maze is represent as a matrix of integer values.
	 */
	@Test
	...
	/**
	 * Test method for
	 * {@link generation.CardinalDirection#getCWConstantForDirection()}.
	 */
	@Test
